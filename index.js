...
const user_location=(req.body.name)
    const getOrganicData = () => {
      const user_location=(req.body.name);//the users location details is received from here
      const userspecific = ("crime"+user_location);
      const weblink=("https://www.google.com/search?q="+userspecific+"&gl=us&hl=en");
      return unirest
...
app.get('/checknamename', function (req, res) {
  if (req.params.name.toLowerCase() === 'homer') {
    res.status(401).send({ message: "Sorry, no Homer's!" });
  } else {
